...
const createFinalMemeFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    let finalMemeImage: MediaPart | undefined;
    const maxRetries = 3;
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (result.media) {
          finalMemeImage = result.media;
          break; // Success, exit loop
        }

        console.log(`Attempt ${i + 1} failed: No media object returned. Retrying...`);
    ...
    if (!finalMemeImage) {
      throw new Error(`Final meme image generation failed after ${maxRetries} attempts.`);
    }
